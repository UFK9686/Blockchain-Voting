...
contract Election {
    ...
    function startelect (string[][] memory candidates string [][] memory party, string [] memory district, uint hour) public {
     
        for(uint i = 0; i<district.length;i++)
        {
        Ballot newBallot = new Ballot (candidates[i], party[i], district[1], msg.sender, hour);
